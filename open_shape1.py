...
def setup(n, k, c):  # generates nxn sized lattice with randomly distributed cxc sized crowders
    ...
    while np.sum(lat) != k:
        # draws randomly between int. not including high int in distri
        x = np.random.randint(0, n - c + 1)
...
def fitO(lat, O1, O2, H90o, n):  # counts how many times open state fits in crowded lattice
    ...
    for i in range(0, Ri):
        for t in range(0, Ci):
            # sum the section of lattice and H90o shape. a value of 2 is a block
            check = lat[i:i + O1, t:t + O2] + H90o
            if np.all(check != 2):
                Wo = Wo + 1.0
    return Wo
...
def calc(Wc, Wo, n, k):
    R = float(Wc) / float(Wo)
    # The 'entropy' difference from the # of configurations of the two states
    dS = np.log(Wc / Wo)
    # fraction of 'volume' that is excluded by the crowders
    Fvc = float(k) / (n * n)
    return R, dS, Fvc
...
# fraction excluded volumes
fracV = np.array([0, 0.025, 0.05, 0.075, 0.1, 0.125, 0.15, 0.2])

expo = np.zeros([len(c), len(fracV) * 4])  # size series for each concentration
# concentration series for each size
expo2 = np.zeros([len(fracV), len(c) * 4])
l2 = 0
for j in range(0, len(fracV)):
    ...
    k = np.zeros([len(c)])
    # k is the # of lattice sites that are excluded.
    k = [(int(fracV[j] * (n * n)) / (i * i)) * (i * i) for i in c]
    ...
    np.savetxt(filename + '_' + str(fracV[j]) + 'trials' + '.CSV',
               expo3, delimiter=',', header='c,trial #,Wo,Wc,R,dS,' * len(c))
    # plt.show()
np.savetxt(filename + 'size_series' + '.CSV', expo, delimiter=',',
           header='Fvc=%r,c,AveR,SD_R,' * len(fracV) % (0, 0.025, 0.05, 0.075, 0.1, 0.125, 0.15, 0.2))
np.savetxt(filename + 'ex_vol_series' + '.CSV', expo2, delimiter=',',
           header='c=%r,Fvc,AveR,SD_R,' * len(c) % (1, 2, 3, 4, 5, 6, 7))
